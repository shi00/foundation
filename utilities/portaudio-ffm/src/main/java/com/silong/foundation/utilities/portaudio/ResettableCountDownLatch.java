 ...
 *
 * <p>A {@code CountDownLatch} is initialized with a given <em>count</em>. The {@link #await await}
 * methods block until the current count reaches zero due to invocations of the {@link #countDown}
 * method, after which all waiting threads are released and any subsequent invocations of {@link
 * #await await} return immediately. This is a one-shot phenomenon -- the count cannot be reset. If
 * you need a version that resets the count, consider using a {@link CyclicBarrier}.
 *
 * <p>A {@code CountDownLatch} is a versatile synchronization tool and can be used for a number of
 * purposes. A {@code CountDownLatch} initialized with a count of one serves as a simple on/off
 * latch, or gate: all threads invoking {@link #await await} wait at the gate until it is opened by
 * a thread invoking {@link #countDown}. A {@code CountDownLatch} initialized to <em>N</em> can be
 * used to make one thread wait until <em>N</em> threads have completed some action, or some action
 * has been completed N times.
 *
 * <p>A useful property of a {@code CountDownLatch} is that it doesn't require that threads calling
 * {@code countDown} wait for the count to reach zero before proceeding, it simply prevents any
 * thread from proceeding past an {@link #await await} until all threads could pass.
 *
 * <p><b>Sample usage:</b> Here is a pair of classes in which a group of worker threads use two
 * countdown latches:
 *
 * <ul>
 ...
 * </pre>
 *
 * <p>Another typical usage would be to divide a problem into N parts, describe each part with a
 * Runnable that executes that portion and counts down on the latch, and queue all the Runnables to
 * an Executor. When all sub-parts are complete, the coordinating thread will be able to pass
 * through await. (When threads must repeatedly count down in this way, instead use a {@link
 * CyclicBarrier}.)
 *
 * <pre>
 * class Driver2 { // ...
 ...
 * </pre>
 *
 * <p>Memory consistency effects: Actions in a thread prior to calling {@code countDown()} <a
...
class ResettableCountDownLatch {
  ...
  private static final class Sync extends AbstractQueuedSynchronizer {
    @Serial private static final long serialVersionUID = 9075231796588256630L;

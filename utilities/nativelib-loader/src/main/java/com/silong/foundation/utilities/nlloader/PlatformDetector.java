 ...
 * @author louis sin
 * @version 1.0.0
 * @since 2023-10-14 10:01
 */
@Slf4j
public class PlatformDetector {
  public static final PlatformDetector PLATFORM_DETECTOR = new PlatformDetector();
  ...
  private final SimpleSystemPropertyOperations systemPropertyOperationProvider =
      new SimpleSystemPropertyOperations();
  private final SimpleFileOperations fileOperationProvider = new SimpleFileOperations();
  ...
  /**
   * Parses a file in the format of {@code /etc/os-release} and return a {@link LinuxRelease} based
   * on the {@code ID}, {@code ID_LIKE}, and {@code VERSION_ID} entries.
   */
  ...
  /**
   * Parses the {@code /etc/redhat-release} and returns a {@link LinuxRelease} containing the ID and
   * like ["rhel", "fedora", ID]. Currently only supported for CentOS, Fedora, and RHEL. Other
  ...
  private static class LinuxRelease {
    ...
    LinuxRelease(String id, String version, Set<String> like) {
      ...
      this.like = unmodifiableCollection(like);
    }
  }

...
public class PersistStorageConfig implements Serializable {
   ...
   * compression is enabled. This parameter can be changed dynamically. Default: 16KB
   */
   ...
   * RocksDB does not enforce * rate limit for anything other than flush and compaction, e.g. write
   * to * WAL. The default value is 64MB.
   */
  @Positive private long rateBytesPerSecond = 10_000_000;

  /**
   * this controls how often tokens are refilled. For * example, * when rate_bytes_per_sec is set to
   * 10MB/s and refill_period_us is set to * 100ms, then 1MB is refilled every 100ms internally.
   * Larger value can * lead to burstier writes while smaller value introduces more CPU * overhead.
   * The default of 100,000ms should work for most cases.
   */
   ...
   * requests come in continuously. This fairness parameter grants * low-pri requests permission by
   * fairness chance even though high-pri * requests exist to avoid starvation. * You should be good
   * by leaving it at default 10.
   */
